...
%
%  [X_SBP_WCnorm, X_SBP_WCcorr,X_SBP_WCsim, X_RMSE]=
%  CFF_apply_wc_refping(fData,params) creates simulated and corrected pings
%  based on FDATA and specified input parameters from the reference ping. 
...
%    rp_pct_amp = Relative reference amplitudes (rp_pct_amp)created by
%  CFF_CREATE_WC_REFPING 
%    rp_pct_range = Relative range (beyond MSR) (rp_pct_range)created by
%  CFF_CREATE_WC_REFPING % rp_pct_range_msra = Relative range (above MSR)
%  (rp_pct_range_msra)created by CFF_CREATE_WC_REFPING  
%    mindB = Minimum WC amplitude value (used for normalisation); Kongsberg
%  default = 63 
%    maxdB = Maximum WC amplitude value (used for normalisation); Kongsberg
%  default = 63 
%
% Outputs include:
...
%   Copyright 2024 Amy Nau. Licensed under MIT
...
for iB = 1:nBlocks
    ...
    for pp = 1:nP
        ...
        % Set buffer to search 10 samples on either side of bottom
        % detection. Select highest amplitude for bottom amplitude and
        % range (Step 3) 
        bd_buff = 10;
        for bb = 1:nB
        ...
        % Calculate relative range for below MSR
        for bb = 1:nB
            for ii = MSS:nS
        ...
        % Calculate maximum bottom amplitude by sample range (Step 6)
        ...
        % Create matrix and infill values
        b = nan(nS,1);
        ...
        % Create "simulated ping" using the max bottom amplitude and the
        ...
        % Create a "corrected ping" by subtracting the simulated ping from
        % the uncorrected ping (Step 9)
        ping_corr = [];
...
% Calculate root mean square error between simualted ping and uncorrected
% ping 
