...
%
%  Apply the function that creates a water column 'Reference Ping' (Nau et
%  al., 2024) 
%
%  [reference_ping, rp_pct_amp, rp_pct_range, rp_pct_range_msra]=
%  CFF_create_wc_refping(fData, params) creates a reference ping based on
%  FDATA and specified input parameters. 
%
...
% Average WC amplitudes across pings to create single "beam pattern" from
% this series of pings (Step 2)
...
% Calculate "relative range" for each sample between bottom detection and
% MSR (Step 5)
for ii = MSS:nS
    for bb = 1:nB
...
% Calculate "percent" of range for each sample above MSR ("msra")
for ii = 1:MSS-1
    for bb = 1:nB
...
% Samples are sorted and unique values taken as the beam pattern is related
% to the amplitude of the highest sample within that sample range, not for
% each beam
...
% Calculate relative reference amplitude  based on maximum amplitude at
% each sample range (Step 7)
