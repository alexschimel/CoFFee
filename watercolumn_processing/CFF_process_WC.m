...
for iFD = 1:nFData
    ...
    if resumeProcessingThisFData
        ...
        % Processed data are double precision (8 bytes) or single precision
        % (4 bytes). Storing this data in this format will take a lot of
        % disk space. To save disk space, we encode that data in a lower
        % precision format (1 or 2 bytes) although we will lose in
        % precision (e.g. -23.4283 dB will be stored as -23.5 dB instead).
        %
        % We define the desired precision of the stored data here:
        % - 1 byte allows storage of 255 different values, allowing for
        %   example a dynamic range of 25.5 dB at 0.1 dB resolution, or 127
        %   dB at 0.5 dB resolution. Aka largest space saving but largest
        %   loss of precision. 
        % - 2 bytes allow storage of 65535 different values, allowing for
        %   example a dynamic range of 655.35 dB at 0.01 dB resolution, or
        %   65.535 dB at 0.001 dB resolution. Aka moderate space saving for
        %   moderate loss of precision.
        %
    ...
    for ig = 1:nMemMapFiles
        ...
        % processing per block of pings in memmap file, in reverse since
        % the last block is the most likely to need updating
        for iB = nBlocks:-1:1
            ...
            % missing). These parameters will be applied to the next fData
            ...
            % recomputation in case there are several blocks of data). This
            % is not ideal. To change eventually XXX
            if nBlocks > 1
        ...
        if nBlocks == 1
            ...
            % look for blocks that didn't use those parameters and reencode
            % them using final parameters 
            for iB = 1:nBlocks
